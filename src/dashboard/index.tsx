  ...
  async function execute(line: string) {
    ...
    try {
      switch (head) {
        ...
        case "edit": append(await runtime.cmdEdit(rest[0], ...rest.slice(1))); break;
        case "appearance": append(await runtime.cmdAppearance(rest[0])); break;
        case "cal": append(await runtime.cmdCal(rest[0], ...rest.slice(1))); break;
        case "privacy": append(await runtime.cmdPrivacy(rest[0])); break;
        case "help": append(":status :why :amnesia <мин> [chatId] :reset :stage <id> :wake [chatId] :debug [chatId] :pause :resume :cringe :relationship :persona :log [YYYY-MM-DD] [chars] :edit persona|speech|vibe|stage|appearance <args> :appearance [show|preview] :privacy [open|owner-only] :cal [list|add|del] :block :unblock :read :clear-chat :report-spam :delete-last :edit-last <text> :sticker :quit"); break;
